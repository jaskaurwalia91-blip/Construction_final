...
DATABASE_URL = os.environ.get('DATABASE_URL', 'postgresql://neondb_owner:npg_C2pgKZH4sSJk@ep-autumn-river-aik6owco.us-east-1.aws.neon.tech/neondb?sslmode=require')
# Database Configuration (NeonDB)
...
def staff_upload_document(project_id):
    if request.method == 'POST':
        ...
        # Fix empty date - convert to None if empty
        if not report_date or report_date.strip() == '':
            report_date = None
        
        # Fix empty description - convert to None if empty
...
def download_file(filename):
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            mimetype = mimetypes_dict.get(file_ext, 'application/octet-stream')
            
            # For images and PDFs, show in browser; for others, download
            as_attachment = file_ext not in ['png', 'jpg', 'jpeg', 'pdf']
            ...
            flash('File not found. Files may have been deleted due to server restart (temporary storage).', 'error')
